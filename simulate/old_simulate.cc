...
plug_test_t plug_test;
plug_init_t plug_init;
plug_update_t plug_update;
const char* lib_filename = "libController.so";
void *libcontroller;

/* TODO: OPEN CV */
unsigned char* color_buffer;
// cv::Mat cv_gray_buffer;
...
void ShowSubCAM(mj::Simulate* sim, mjrRect rect, mjvScene* scn, mjvOption* opt, mjvPerturb* pert){
  ...
  color_buffer = (unsigned char*) malloc(viewport.height*viewport.width * 3 * sizeof(unsigned char));
  // gray_buffer = (unsigned char*) malloc(viewport.height*viewport.width * 3);
  depth_buffer = (float*) malloc(viewport.height*viewport.width * 1 * sizeof(float));
  mjr_readPixels(color_buffer, depth_buffer,viewport, &sim->platform_ui->mjr_context());

  // cv::waitKey(0);
  depth8 = (unsigned char *)malloc(viewport.height * viewport.width * 3 * sizeof(unsigned char));
  ...
  if ((state->dragrect==sim->ui0.rectid) ||
    ...
    // simulation section
      ...
      case 1:             // Reset
        ...
        sim->reload_controller_plug_bool.store(true); /*** AUVC ***/
        break;
...
/*** AUVC ***/
void Simulate::updatePlugSimulate(mjModel* m, mjData* d, void* libcontroller, const char * lib_filename) {
  dlclose(libcontroller);
  printf("[INFO] WAITED ON LOCK | FILE: %s | Line: %d\n", __FILE__, __LINE__);
  printf("closing libcontroller.........\n");
  libcontroller = dlopen(lib_filename, RTLD_NOW);
  printf("opened libcontroller.........\n");
  plug_update = (plug_update_t)dlsym(libcontroller, "updatePlug");
  printf("plug_update loaded.........\n");
  if (plug_update == NULL){
    fprintf(stderr, "ERROR: (%d) could not load symbol updatePlug() from %s: %s\n", __LINE__, lib_filename, dlerror());
    dlclose(libcontroller);
  }
  plug_update(m,d);
  ...
  {
    MutexLock lock(mtx);
    this->reload_controller_plug_bool = 1;
    printf("[INFO] LOCKED | FILE: %s | Line: %d\n", __FILE__, __LINE__);

    // clear request
    this->reload_controller_plug_bool = 0;
    this->cond_reload_plug_bool.notify_all();

    // Wait for the render thread to be done loading
    // so that we know the old model and data's memory can
    // be free'd by the other thread (sometimes python)
    cond_reload_plug_bool.wait(lock, [this]() { return this->reload_controller_plug_bool == 0; });
    printf("[INFO] WAITED ON LOCK | FILE: %s | Line: %d\n", __FILE__, __LINE__)r
  }
}

/*** AUVC ***/
bool Simulate::reloadLibPlug(mjModel* m, mjData* d){
    ...
    plug_test = (plug_test_t)dlsym(libcontroller, "test_func");
    if (plug_test == NULL){
    ...
    // plug_test();
    plug_init = (plug_init_t)dlsym(libcontroller, "initPlug");
    if (plug_init == NULL){
...
void Simulate::Render() {
  ...
  if(this->ui0_enable){
    const_viewport.left = rect.width - rect.left - const_viewport.width/2;
  }
  if (this->profiler) {
